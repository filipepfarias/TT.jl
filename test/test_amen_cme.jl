begin
    ...
    Ap = Vector{Array}(undef, d)
    # Cascadic MPO -- production reactions
    Ap[1] = zeros(1, n[1], n[1], 3)
    Ap[1][1, :, :, 1] = I(n[1])
    Ap[1][1, :, :, 2] = diagm((0:n[1]-1) ./ (5 .+ (0:n[1]-1)))
    Ap[1][1, :, :, 3] = (diagm(-1 => ones(n[1] - 1)) - I(n[1])) * 0.7 * diagm([ones(n[1] - 1); 0])
    for i = 2:d-1
        Ap[i] = zeros(3, n[i], n[i], 3)
        Ap[i][1, :, :, 1] = I(n[i])
        Ap[i][3, :, :, 3] = I(n[i])
        Ap[i][1, :, :, 2] = diagm((0:n[i]-1) ./ (5 .+ (0:n[i]-1)))
        Ap[i][2, :, :, 3] = (diagm(-1 => ones(n[i] - 1)) - I(n[i])) * diagm([ones(n[i] - 1); 0])
    end
    Ap[d] = zeros(3, n[d], n[d])
    Ap[d][2, :, :] = (diagm(-1 => ones(n[d] - 1)) - I(n[d])) * diagm([ones(n[d] - 1); 0])
    Ap[d][3, :, :] = I(n[d])
    # Laplace MPO -- destruction reactions
    Ad = Vector{Array}(undef, d)
    Ad[1] = zeros(1, n[1], n[1], 2)
    Ad[1][1, :, :, 1] = (diagm(1 => ones(n[1] - 1)) - I(n[1])) * diagm((0:n[1]-1)) * 0.07
    Ad[1][1, :, :, 2] = I(n[1])
    for i = 2:d-1
        Ad[i] = zeros(2, n[i], n[i], 2)
        Ad[i][1, :, :, 1] = I(n[i])
        Ad[i][2, :, :, 2] = I(n[i])
        Ad[i][2, :, :, 1] = (diagm(1 => ones(n[i] - 1)) - I(n[i])) * diagm((0:n[i]-1)) * 0.07
    end
    ...
    # tic;
    # U_ex = amen_solve2(B, f, tol * 1e-3; x0=U0, nswp=nswp * 3, kickrank=kickrank, trunc_norm=trunc_norm, verb=true, max_full_size=50, local_restart=50, local_iters=2, resid_damp=2)
    U_ex = amen_solve2(B, f, tol * 1e-3; x0=U0, nswp=2, kickrank=kickrank, trunc_norm=trunc_norm, verb=true, max_full_size=50, local_restart=50, local_iters=2, resid_damp=2)
    # toc;
end
