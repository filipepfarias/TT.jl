...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
"""
function tt_qtoepl(x, d)
    D = sum(length,d);
    tt = tt_qshiftstack(d);
    
    sz=Matrix{Int}(undef,0,2);
    for K=1:D
        sz = [sz; 4*ones(Int,d[K]) 2*ones(Int,d[K]); 1 2];
    end
    ...
    for K = 1:D
        ...
        tt1[ind1 + d[K]] = reshape(reshape(permutedims(tt[ind+d[K]],[2, 1, 3]),4*p,q)*reshape(permutedims(tt[ind+d[K]+1],[2, 1, 3]),q,r),4,p,r);
        tt1[ind1 + d[K]] = permutedims(tt1[ind1 + d[K]], [2, 1, 3]);
        ind=ind+d[K]+1;
        ind1=ind1+d[K];
    end
    tt = tt1;
    # tt1 = tt_tensor;
    tt = vector_to_core(TTTensor, tt);
    tt = TTMatrix(tt);
    return tt
    end
